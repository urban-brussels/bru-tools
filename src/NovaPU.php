...
class NovaPU extends Main
{
    ...
    {
        if (is_null($geom)) {
            return 's_iddossier=000000';
        };
        if (!is_null($buffer) && !stristr($geom, "POINT")) {
        ...
        foreach ($json->features as $key => $value) {
            ...
            foreach ($property as $k => $v) {
                ...
                elseif (in_array($k, ['mpp', 'aviscbe', 'avisfd', 'aviscc'])) {
                    if ($v === 'Non' || $v === 'false' || $v === false) {
                        $property[$k] = "false";
                    } elseif ($v === 'Oui' || $v === 'true' || $v === true) {
                        $property[$k] = "true";
                    } else {
                        $property[$k] = null;
                    };
            ...
            if (isset($property['datedebutmpp'])) {
                ...
                $datetime2 = new \DateTime($property['datefinmpp'] ?? null);
                $datetime2->add(new \DateInterval('P1D'));  /* Add 13 hours... Nova WFS at 11am */
    ...
    public static function getPermitHistory(?string $wkt, string $refnova): ?array
    {
        if(is_null($wkt)) {
            return [];
        }

        $history = self::getPermitsData(self::filterByGeometry($wkt), 31370, "refnova,s_iddossier,datedepot,datenotifdecision,etatfinal,statutpermisfr");
        if(is_null($history)) {
            return [];
        }

        foreach ($history as $k => $v) {
            if ($v['refnova'] === $refnova) {
                unset($history[$k]);
            }
        }
        ...
        $date_tri = array();
        foreach ($history as $k => $v) {
            $date_tri[$k] = $v['datedepot'];
        }
        array_multisort($date_tri, SORT_DESC, $history);
